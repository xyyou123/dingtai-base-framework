    ...
    public void GetOpenPicByStID(LaunchAdModel model) {
        if (model != null) {
            ...
            if (model.getOpenPicDetail() != null && model.getOpenPicDetail().size() > 0) {
                ...
                for (LaunchAdDetailsModel model1 : model.getOpenPicDetail()) {
                    ad = new ADModel();
                    ad.setADFor(model1.getADFor());
                    ad.setLinkTo(model1.getLinkTo());
                    ad.setLinkUrl(model1.getLinkUrl());
                    ad.setLiveChannel(model1.getLiveChannel());
                    ad.setADName(model1.getOpenPicName());
                    ad.setImgUrl(model1.getImgUrl());
                    adlist.add(ad);
                }
...
//                load(mCurrentAd);
                ...
                ArrayList<ADModel> adlist1 = new ArrayList<>();
                adlist1.add(ad);
                load(adlist1);
            }
        }
    ...
    protected void load(final List<ADModel> list) {
        if (list != null && list.size() > 0) {
            ...
            Glide.with(imageAd).load(list.get(0).getImgUrl()).apply(options)
                    .into(new SimpleTarget<Drawable>() {
                        @Override
                        public void onResourceReady(@NonNull Drawable resource, @Nullable Transition<? super Drawable> transition) {
                            if (imageAd != null) {
                                float r = getRate(resource);
                                //todo  加载滚动图

                                mBanner = AdvertisementView.createInRecyclerView(LaunchAdListActivity.this, r);
                                mBanner.setBannerStyle(BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE);
                                mBanner.setOnBannerListener(LaunchAdListActivity.this);
                                ll_container.addView(mBanner, 0);


                                adList = list;
                                mBanner.setVisibility(View.VISIBLE);
                                AdvertisementView.attach(mBanner, list);
                                mBanner.start();
                            }
                        }
                    });
    ...
    protected float getRate(@NonNull Drawable resource) {
    ...
    public void OnBannerClick(int position) {
        ...
        NewsNavigation.adNavigation(adList.get(position));
    }
}
