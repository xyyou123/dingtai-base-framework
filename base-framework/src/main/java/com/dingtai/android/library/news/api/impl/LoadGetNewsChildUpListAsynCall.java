 ...
 * date:2018-08-21
 */
public class LoadGetNewsChildUpListAsynCall extends AbstractAsynCall<List<NewsListModel>> {

    private static final String URL = "base";

    @Inject
    public LoadGetNewsChildUpListAsynCall(){}
    ...
    public Observable<List<NewsListModel>> call(AsynParams params) {
		...
		//业务逻辑
        return http().call(NewsApi.class, URL).loadGetNewsChildUpList(top, dtop, Resource.API.SIGN, chid, Resource.API.STID).subscribeOn(Schedulers.io())
                ...
                .onErrorResumeNext(new Function<Throwable, ObservableSource<? extends List<NewsListModel>>>() {
                    ...
                    public ObservableSource<List<NewsListModel>> apply(Throwable throwable) {
                        return Observable.just(database().call(NewsListModelDao.class)
                                .queryBuilder()
                                ...
                                .limit(NumberUtil.parseInt(top, 0))
                                .list());
                    }
                })
