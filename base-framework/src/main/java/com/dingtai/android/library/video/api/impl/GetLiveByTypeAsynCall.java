 ...
 * date:2018-08-30
 */
public class GetLiveByTypeAsynCall extends AbstractAsynCall<List<LiveChannelModel>> {

    private static final String URL = "base";

    @Inject
    public GetLiveByTypeAsynCall(){}
    ...
    public Observable<List<LiveChannelModel>> call(AsynParams params) {
		...
		final String channelType = params.get("channelType");
		//业务逻辑
        return http().call(VideoApi.class, URL)
                .GetLiveByType(type, top, dtop, Resource.API.STID, Resource.API.SIGN,channelType)
                ...
                .map(new Function<JSONObject, List<LiveChannelModel>>() {
                    ...
                    public List<LiveChannelModel> apply(JSONObject object) {
                        ...
                        dao.queryBuilder().where(LiveChannelModelDao.Properties.ID.eq(type)).buildDelete().executeDeleteWithoutDetachingEntities();
                ...
                .onErrorResumeNext(new Function<Throwable, ObservableSource<? extends List<LiveChannelModel>>>() {
                    ...
                    public ObservableSource<? extends List<LiveChannelModel>> apply(Throwable throwable) {
                        return Observable.just(database().call(LiveChannelModelDao.class)
                                .queryBuilder()
                                .where(LiveChannelModelDao.Properties.ID.eq(type))
