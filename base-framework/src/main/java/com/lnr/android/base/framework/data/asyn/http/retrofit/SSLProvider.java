...
public final class SSLProvider {
    ...
    public static SSLParams createSSLSocketFactory(Context context, String serverCerStr, String bksPath, String password) {
        SSLParams sslParams = null;
        try {
            InputStream serverCer = new Buffer().writeUtf8(serverCerStr).inputStream();
            InputStream clientBks = context.getAssets().open(bksPath);
            //初始化https证书参数信息
            sslParams = getSslSocketFactory(new InputStream[]{serverCer}, clientBks, password);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static SSLParams getSslSocketFactory(InputStream[] certificates,
