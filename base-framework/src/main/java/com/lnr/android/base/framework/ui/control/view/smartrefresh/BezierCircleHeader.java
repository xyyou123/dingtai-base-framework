...
public class BezierCircleHeader extends InternalAbstract implements RefreshHeader {
    ...
    public static class Text {
        ...
        private int centerColor, outerColor;

        public Text(String left, String center, String right, int centerColor, int outerColor) {
            this.left = left;
            this.center = center;
            this.right = right;
    ...
    public BezierCircleHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        thisView.setMinimumHeight(DensityUtil.dp2px(72));
        ...
        mTextPaint.setTextSize(DimenUtil.sp2px(context, 20));
        mTextPaint.setAntiAlias(true);
    }

    public void setText(Text... texts) {
        this.mTextList = Arrays.asList(texts);
    }
    ...
    private void drawBoll(Canvas canvas, int viewWidth) {
        if (mShowBoll) {
            ...
            if(mTextList != null && mTextList.size() > 0) {
                float outerR = mBollRadius + mOuterPaint.getStrokeWidth() * 2.0F;
                float yPos = canvas.getHeight() / 2 - (mTextPaint.descent() + mTextPaint.ascent()) / 2.0F;

                Text text = mTextList.get(mTextIndex);
                mTextPaint.setColor(text.centerColor);
                canvas.drawText(text.center, viewWidth / 2 - mTextPaint.measureText(text.center) / 2.0F, yPos, mTextPaint);

                mTextPaint.setColor(text.outerColor);
                canvas.drawText(text.left, (float)(viewWidth / 2) - mTextPaint.measureText(text.left) - outerR - 10.0F, yPos, mTextPaint);
                canvas.drawText(text.right, (float)(viewWidth / 2) + outerR + 10.0F, yPos, mTextPaint);
            }
        }
    }
...
//    }
    ...
    public void onReleased(@NonNull RefreshLayout refreshLayout, int height, int maxDragHeight) {
        ...
        mTextIndex = (mTextIndex + 1) % mTextList.size();
    }
