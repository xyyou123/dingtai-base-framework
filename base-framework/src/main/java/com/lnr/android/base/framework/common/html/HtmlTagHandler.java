...
import com.lnr.android.base.framework.common.html.span.NumberSpan;
...
class HtmlTagHandler implements Html.TagHandler {
    ...
    private VideoSpan mLastVideoSpan;

    HtmlTagHandler(HtmlImageGetter htmlImageGetter) {
        this.mHtmlImageGetter = htmlImageGetter;
    }
    ...
    public void handleTag(final boolean opening, final String tag, Editable output, final XMLReader xmlReader) {
        if (opening) {
            ...
            if (tag.equalsIgnoreCase(UNORDERED_LIST)) {
                ...
                startVideo(output, xmlReader, mHtmlImageGetter);
            } else if(tag.equalsIgnoreCase("source") && mLastVideoSpan != null) {
                startVideoSource(xmlReader);
            }
        } else {
            // closing tag
            if (tag.equalsIgnoreCase(UNORDERED_LIST)) {
    ...
    private void startVideo(Editable text, XMLReader xmlReader, Html.ImageGetter img) {
        Map<String, String> attributes = getAttributes(xmlReader);
        String src = attributes.get("poster");
        Drawable d = null;
        ...
        int len = text.length();
        text.append("\uFFFC");
        mLastVideoSpan = new VideoSpan(mContext, d, src);
        text.setSpan(mLastVideoSpan, len, text.length(),
                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    private void startVideoSource(XMLReader xmlReader) {
        Map<String, String> attributes = getAttributes(xmlReader);
        mLastVideoSpan.setVideoPath(attributes.get("src"));
        mLastVideoSpan = null;
    }
